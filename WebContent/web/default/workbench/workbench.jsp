  ...
  <% if ("true".equals(DOGlobals.getValue("multi.tenancy"))) { %>
     ...
     <%} %>
       ...
       <div class="live-tile cobalt" >
	    ...
	    <div class="live-tile cobalt" >
				 	<div>
				 		<a  href="javascript:loadPml({'pml':'PM_do_globals_config','target':'_opener_tab','title':'<%=I18n.instance().get("参数管理")%>'})" >
				 			   <img class="full" src="<%=request.getContextPath()%>/web/default/workbench/images/Parameter.png"/>
				 		</a>	   
				 	</div>
			  	 	<a class="tile-title accent"  href="javascript:loadPml({'pml':'PM_do_globals_config','target':'_opener_tab','title':'<%=I18n.instance().get("参数管理")%>'})" ><%=I18n.instance().get("参数管理")%></a>
	    </div>
...
<!-- 第一层Tools -->
	<div class="tiles tile-group seven-wide">
	  <div class="tile-strip seven-wide">
				<div class="live-tile cobalt" >
				 	<div>
				 		<a  href="javascript:loadPml({'pml':'PM_do_ui_controller_Main','target':'_opener_tab','title':'<%=I18n.instance().get("View Component Manager")%>'})">
				 			   <img class="full" src="<%=request.getContextPath()%>/web/default/workbench/images/controller.png"/>
				 		</a>	   
				 	</div>
			  	 	<a class="tile-title accent" href="javascript:loadPml({'pml':'PM_do_ui_controller_Main','target':'_opener_tab','title':'<%=I18n.instance().get("View Component Manager")%>'})"><%=I18n.instance().get("View Component Manager")%></a>
	            </div>
			    
				<div class="live-tile cobalt">
					<div><a  href="javascript:loadPml({'pml':'PM_do_actionconfig_Main','target':'_opener_tab','title':'<%=I18n.instance().get("Action Manager")%>'})">
					 <img class="full" src="<%=request.getContextPath()%>/web/default/workbench/images/Actionenter.png"/>
					</a>
				   </div>
				   <a class="tile-title accent"  href="javascript:loadPml({'pml':'PM_do_actionconfig_Main','target':'_opener_tab','title':'<%=I18n.instance().get("Action Manager")%>'})"><%=I18n.instance().get("Action Manager")%></a>
				</div>
				<div class="live-tile cobalt">
				 	<div><a  href="javascript:loadPml({'pml':'PM_do_resource_Main','target':'_opener_tab','title':'<%=I18n.instance().get("Resource Manager")%>'})">
				 	 <img class="full" src="<%=request.getContextPath()%>/web/default/workbench/images/Configure.png"/>
				 	</a>
					</div>
					<a class="tile-title accent" href="javascript:loadPml({'pml':'PM_do_resource_Main','target':'_opener_tab','title':'<%=I18n.instance().get("Resource Manager")%>'})"><%=I18n.instance().get("Resource Manager")%></a>
				</div>
			    <div class="live-tile cobalt">
			...
			<div class="live-tile cobalt ">
				<div>
					<a  href="javascript:loadPml({'pml':'PM_do_jobconfig_result','target':'_opener_tab','title':'<%=I18n.instance().get("Job Config")%>'})">
						 <img class="full" src="<%=request.getContextPath()%>/web/default/workbench/images/Devices.png"/>
					</a>
				</div>
