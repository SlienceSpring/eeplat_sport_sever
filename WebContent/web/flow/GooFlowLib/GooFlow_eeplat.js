...
// 随即id
function guid(){
	var str1 = 'xxxxxxxxxxxxxxxxyxxxxxxxxxxxxxxx';
	var str2 = str1.replace(/[xy]/g,function(c){
...
//初始化区域图的对象
	...
	if(this.$editable){
	  this.$workArea.on("click",{inthis:this},function(e){
		...
		This.addNode(guid(),{name:"new node",left:X,top:Y,type:This.$nowType});
		This.$max++;
	  });
	  //划线或改线时用的绑定
...
GooFlow.prototype={
	...
	initGroup:function(width,height){
      ...
      e.data.inthis.addArea(guid(),{name:"new area",left:X,top:Y,color:color[e.data.inthis.$max%4],width:200,height:100});
      e.data.inthis.$max++;
      return false;
		}
	  });
	...
	addNode:function(id,json){
		...
		if(typeof json.left == 'string'){
			json.left = parseInt(json.left);
		}
		if(typeof json.top == 'string'){
			json.top = parseInt(json.top);
		}
	...
	initWorkForNode:function(){
		...
		this.$workArea.delegate(".GooFlow_item","mouseup",{inthis:this},function(e){
			...
			if(lineStart&&!This.$mpTo.data("p")){
				This.addLine(guid(),{from:lineStart.id,to:this.id,name:""});
				This.$max++;
			}
			else{
				if(lineStart){
