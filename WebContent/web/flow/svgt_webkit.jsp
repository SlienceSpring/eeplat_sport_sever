...
//////////////////////////////////////////////////////////////////////////////////					
				 ...
				 $("#workflow",svg.root()).bind("mouseup",function(evt){
					...
					////加载xml
//					$("#editproperty",svg.root()).bind("click",function(evt){
//						if(sedlectedLine){///现在只是简化处理，只针对选中的文本，需要线文本和线之间建立管理
//							showDialog('<%=request.getContextPath()%>/pane_wf_propertydetailsline.pml?isApp=true&vid=') ;
//						}else if(selectedNodeBak){
...
function doMouseMove(evt,svg){
	...
	console.log("2nodeMoving:::" + nodeMoving );
	//不再移动
	if(nodeMoving==false){
		return;
	}
...
function doMouseUp(evt){
	...
	nodeMoving = false;
	var o = evt.target;
	console.log("up nodeMoving::: false" );
...
function addNewNode(aNode)//增加新节点,user流默认property为none
{
  ...
  try{	
				...
				////绑定mousedown 选中元素
				shape.setAttribute("onmousedown", 'doClick(evt)');
				shape.setAttribute("onmouseup", 'doMouseUp(evt)');
				///绑定单击事件				
...
<span style="font-size:12px"><%=I18n.instance().get("(创建新节点的方式不是拖动，而是点击、点击。)")%></span>
<div>
<br/>

<div id="svgbasics" unselectable="on" onselectstart="return false;" style="-moz-user-select:none;">
	﻿<svg style=" width: 825px; height: 1095px; " xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" id="workflow" >
		...
		<!-- 	<image id="editproperty"  x="250" y="20" width="56" height="20" xlink:href="images/property<%=enPrix%>.gif" style="cursor:hand" canmove="false" type="bk" style="cursor:pointer" />  -->
