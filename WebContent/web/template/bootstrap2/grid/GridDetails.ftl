		 ...
		 <#list mapForms?keys as aKey>
			...
			<#list theItems as item>
				...
				<#if (item.nameColspan?exists && item.nameColspan == 0) >
				    <td  <#if item.noWrapValue>nowrap='nowrap'</#if>   <#if (item.newLine || (item_index+1)==theItems?size  || theItems[item_index+1].newLine )> colspan="${(colNum-i-1)*2+2}"</#if> style='${item.style?if_exists}' <@visibleEditor item/> > ${htmlValue} <@editable item/></td>
				<#else>                                                 
				    <th class="tdkey text-nowrap"  style="width:100px"> ${item.l10n}</th> 
				    <td  <#if item.noWrapValue>nowrap='nowrap'</#if>   <#if (item.newLine || (item_index+1)==theItems?size  || theItems[item_index+1].newLine )> colspan="${(colNum-i-1)*2+1}"</#if> style='${item.style?if_exists}' <@visibleEditor item/>>  ${htmlValue} <@editable item/></td>
				</#if>     
			    <#assign i = i + 1/>
			    </#if>
			</#list>
