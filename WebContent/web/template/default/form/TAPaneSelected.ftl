	  ...
	  $('#${model.objUid}${model.data.uid}').bind('click',function(){
		    ...
		    loadPml({
  			 <#if (model.linkPaneModel?exists && model.linkPaneModel.linkType==5)>
		   			 ...
		   			 'paras':'dataBus=setContext&contextKey=${model.data.bo.name}&contextValue=${model.data.uid}&contextNIUid=${(model.data.map.contextniuid)?if_exists}&contextPIUid=${model.data.map.contextpiuid?if_exists}&parentPaneName=${model.gridModel.containerPane.name}',
