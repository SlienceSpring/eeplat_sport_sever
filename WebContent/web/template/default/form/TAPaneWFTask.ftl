  ...
  $('#${model.objUid}${model.data.uid}').bind('click',function(){
	    ...
	    loadPml({
  			     <#if (model.linkPaneModel?exists && model.linkPaneModel.linkType==5)>
	   			 	...
	   			 	'pmlName':'${paneModel.name}',
	   			 	'pmlWidth':'${paneModel.paneWidth?if_exists}',
	   			 	'pmlHeight':'${paneModel.paneHeight?if_exists}',

	   			 'paras':'dataBus=setContext&contextKey=do_wfi_nodeinstance&contextValue=${model.data.uid}&contextKey=${busiBOName?if_exists}&contextValue=${instance_uid?if_exists}&parentPaneName=${model.gridModel.containerPane.name}',
