<#include "TFormBase.ftl">
 <button  type="button"    id='${model.objUid}' class='save btn btn-default'>${model.l10n?default("保存")}</button>
 <script>
 ...
 $('#${model.objUid}').bind('click',function(){
		 ...
		 // ///提示性问题
		    var echoJs = "${model.echoJs!}";
		    
			if (echoJs != null && echoJs != ''  && !eval(echoJs)) {
				return;
			}
	 		        ...
	 		        ///////////////////////modify 
	 		        	if($(this).attr("edit")=='true'){
	 		        		if($(this).attr("value")==null){//add
	 		        		   addStr = addStr + O2String($(this).find("input, select, textarea").serializeArray()) + ",";
	 		        		   if(inputName==""){
	 		        		   	 inputName = $(this).find("input, select, textarea").eq(0).attr("name");
	 		        		   }
	 		        		}else{//update
	 		        		   modiStr = modiStr +  $(this).attr("value") +":" +  O2String($(this).find("input, select, textarea").serializeArray()) + ",";
	 		        		}
	 		        	}		        
	 		        }
	 		     );
