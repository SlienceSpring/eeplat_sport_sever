							...
							<TD>
								<TABLE  cellSpacing=0 cellPadding=0>
									<TBODY>
										<TR class="">
											<TD class="b_left"></TD>
											<TD class="b_center"> <#if '${dataBind(data,item)}' ==''>${item.htmlValue}</#if></TD>
											<TD class="b_right"></TD>
										</TR>
									</TBODY>
								</TABLE>
	...
	<table id='g${model.objUid}' class='tablesorter' border="0" cellpadding="1" cellspacing="1" >
		...
		<tbody>
			...
			<#list model.abstractGridFormLinks as item>
				...
				<#else>                                                 
				    <td class='tdkey' <#if item.noWrapLabel>nowrap='nowrap'</#if>>${item.l10n} <@JudgeIsNull item/></td> 
