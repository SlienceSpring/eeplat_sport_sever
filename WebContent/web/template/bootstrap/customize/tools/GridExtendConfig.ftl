...
<script>
   ...
   function calCombitionLogic(lastIndex){
	...
	function criteriaColChange(thiso,vflag){
        ...
        var htmlUrl = globalURL +	"web/default/htmlform.jsp?formUid=" + formUid;
        if(vv!=undefined && vv!="" ){
        	htmlUrl = htmlUrl + "&instanceuid=" + vv; 
        } 
		$(thiso).nextAll(".uicomponent").load(
		   ...
		   function(){
		       if(vflag){
					...
					if(vv=='%SYSTEM%'){
					   	$(thiso).nextAll(".uicomponent").find("select").attr('value','%SYSTEM%');
					   	$(thiso).nextAll(".uicomponent").find("option:eq(0)").val('%SYSTEM%').text('%SYSTEM%').attr("selected",true);
	...
	$(".div_selectcols").delegate(".selectoperator","change",function(){
		...
		|| $(this).val()=='afternow'
			...
			$(this).nextAll(".uicomponent").find("option:eq(0)").val('%SYSTEM%').text('%SYSTEM%').attr("selected",true);
			$(this).nextAll(".uicomponent").find("select").attr('value','%SYSTEM%');
			...
			if($(this).nextAll(".uicomponent").find(":input").val() =='%SYSTEM%' ){
				...
				$(this).nextAll(".uicomponent").find("select").val('');
			}
		}
