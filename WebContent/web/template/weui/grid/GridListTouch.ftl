...
<div class="panel  panel-info panel-block ">
    <div class="panel-heading">    
      			<#if model.caption?exists>
      			 <a data-toggle="collapse"  href="#co${model.objUid}"   aria-expanded="true" >
 				${model.caption}
 				</a>
 				</#if>
    </div>
    ...
    <div id="co${model.objUid}" class="panel-collapse collapse in table-responsive" >
		...
		<table id='g${model.objUid}'  class="table table-bordered" style="cursor:pointer">
			<thead>
			  <tr>
				<#--隐藏列，数据部分输出记录的主键-->
	...
	<#if (model.bottomOutGridFormLinks?size > 0  || (model.rowSize?exists && model.rowSize > 0)) >
	<div class="gridchildfooter">
...
/////tr click
		$('#g${model.objUid}').delegate('tbody tr','click',function(e){
			...
			var invokeId = '${model.normalGridFormLinks[model.normalGridFormLinks?size - 1].objUid}' +  $(this).attr("value");
			$("#" + invokeId).click();
		});
