...
public class DOGlobals {
	 ...
	 * 全局配置常量
	 */
	private static Map<String, String> globalConfigs = new HashMap<String, String>();
	...
	static {
		...
		try {
			...
			try {
				aPath = java.net.URLDecoder.decode(aPath, "utf-8");
			} catch (Exception e) {
...
//			loadConifg();
			...
			System
			.out.println("All global config::" + globalConfigs);
			//globalConfigs.put(attrName, nodeValue);
			// ///////////执行定时统计
			// if (IS_SCHEDULE == 1) {
	...
	public static void loadConifg() {
		try {
			DOService globalsService = DOService.getService("do_globals_list");
			if (globalsService != null) {
				List<BOInstance> bis = globalsService.invokeSelect();
				for (BOInstance bi : bis) {
					globalConfigs.put(bi.getValue("dokey"), bi.getValue("dovalue"));
				}
			}
		} catch (Exception e) {
			// TODO Auto-generated catch block
	...
	public static void changeLocal(String aLocal) {
		globalConfigs.put("lang.local", aLocal);
	}
	
	public static Map<String, String> getReadOnlyConfigs(){
		return Collections.unmodifiableMap(globalConfigs);
	}
	...
	public BOInstance getThreadInstance(){
		return this.getRuleContext().getInstance();
	}
