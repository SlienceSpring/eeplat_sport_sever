...
public class SSOController extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request,
		...
		// //重试3次以后显示验证码
		Object trySize = request.getSession().getAttribute("trysize");
		if (trySize == null) {
			request.getSession().setAttribute("trysize", Integer.valueOf(1));
		}
		...
		if (returnValue != null && !returnValue.trim().equals("")) {
			if (returnValue == "randerr") {
				echoStr = I18n.instance().get("验证码不正确！");
			} else if (returnValue == "overtry") {
				echoStr = I18n.instance().get("重试次数过多！");
			} else {
		...
		outHtml.append("\",\"echoStr\":\"").append(echoStr);
		outHtml.append("\",\"returnValue\":\"").append(returnValue);
		outHtml.append("\"}");
		...
		// //改变所用的jslib
		if ("jquerymobile".equals(formBI.getValue("mobileclient"))) {
			if (DOGlobals.getInstance().getSessoinContext().getUser() != null) {
				...
				DOGlobals
						...
						.getSessoinContext()
						.getUser()
						.putValue("user_login_device", WebStyle.TERMINAL_MOBILE);
			}
			log.info("use jslib " + WebStyle.TERMINAL_MOBILE);
		} else if ("true".equals(formBI.getValue("mobileclient"))) {
			if (DOGlobals.getInstance().getSessoinContext().getUser() != null) {
				DOGlobals
						.getInstance()
						.getSessoinContext()
	...
	public String makeSimpleLogin(HttpServletRequest request, BOInstance formBI) {
		...
		String randstr = judgeRand(request, formBI);
	...
	public String[] makeMultiProjectsLogin(HttpServletRequest request,
		...
		for (DOApplication doa : doas) {
			...
			// .putValue("default_app_uid", doa.getObjUid());
			// }
	...
	public String[] makeMultiLoginAndTenant(HttpServletRequest request,
		...
		log.info("当前登录的租户为::" + tenant);

		if (tenant != null
				&& (tenant.getValue("multi_datasource_uid") == null || tenant
						.getValue("model_datasource_uid") == null)) {

			ret[0] = I18n.instance().get("您的账号还没有激活！");
			return ret;
		}
		...
		if (ma == null) {
			...
			user.putValue("asrole", ma.getAsrole());
		}
		...
		if (DOGlobals.getInstance().getSessoinContext().getUser() != null) {
			DOGlobals.getInstance().getSessoinContext().getUser()
					.putValue("default_app_uid", doas.get(0).getObjUid());
		}
	...
	private String judgeRand(HttpServletRequest request, BOInstance formBI) {
		// //首先判断验证码，手机访问不判断

		Integer trysize = (Integer) request.getSession()
				.getAttribute("trysize");

		log.info("trysize:::" + trysize);

		if (trysize <= 3) {
			trysize = trysize + 1;
			request.getSession().setAttribute("trysize", trysize);
			return null;
		} else if (trysize == 4) {
			trysize = trysize + 1;
			request.getSession().setAttribute("trysize", trysize);
	...
	public static boolean isJqueryMobile() {

		try {
			if (DOGlobals.getInstance().getSessoinContext().getWebStyle()
					.getStyleType().equals(WebStyle.TERMINAL_MOBILE)) {
				return true;
			}
			return false;
