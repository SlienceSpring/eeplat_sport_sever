package com.exedosoft.plat.weixin.passivity.msg;

import org.w3c.dom.Document;

import com.exedosoft.plat.weixin.passivity.WXXmlElementName;
...
public class Msg4Event extends Msg {
	...
	public void read(Document document) {
		...
		//用户未关注时，进行关注后的事件推送
		//用户已关注时的事件推送
		if(SUBSCRIBE.equalsIgnoreCase(this.event) || UNSUBSCRIBE.equalsIgnoreCase(this.event) || SCAN.equalsIgnoreCase(this.event)){
		...
		}else if(LOCATION.equalsIgnoreCase(this.event)){// 上报地理位置事件
		...
		}else if(CLICK.equalsIgnoreCase(this.event)){// 自定义菜单事件
			this.eventKey = getElementContent(document, WXXmlElementName.EVENT_KEY);
		}
	}
