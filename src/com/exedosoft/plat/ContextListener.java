...
import com.exedosoft.plat.action.customize.tools.DOImport;
...
import com.exedosoft.plat.util.ScanFile;
...
public class ContextListener implements ServletContextListener {
	...
	public void contextInitialized(ServletContextEvent arg0) {
		...
		try {
			// ///////////////////////自动升级机制。
			// //从升级目录读取文件，和配置库进行对比，如果配置库中没有，加载升级文件。
			// /读完删除文件
			String baseDir = DODataSource.getConfigDBPath() + "../uprade";

			log.info("升级文件目录::" + baseDir);

			ScanFile s = new ScanFile();

			List<String> files = s.parse(baseDir, new String[] { ".xml" },
					false);

			for (String fileName : files) {

				String filePath = baseDir + "/" + fileName.trim();
				File theFile = new File(filePath);
				System.out.println("FileName::" + filePath);
				boolean isImport = true;
				if (filePath.indexOf(".sql.xml") != -1) {
					DOImportSQL.importSQL(filePath);
				} else {
					DOImport doi = new DOImport();
					doi.importXml("", filePath);
				}
				StringUtil.copyFile(theFile, new File(baseDir + "/bak/"
						+ fileName));
				theFile.delete();
			}
