...
public class SessionParterYH implements SessionParter {
	...
	public List getParterAuths(String accountUid) {
		...
		DOService dos = DOService.getService("do_org_user_browse_byuid");
		if(dos!=null){
			try {
				List ls = dos.invokeSelect(user.getUid());
				if(ls.size() > 0){
					BOInstance bi = (BOInstance)ls.get(0);
					String unituid = bi.getValue("deptuid");
					if(unituid !=null && !unituid.trim().equals("")){
						//添加单位部门权限验证
						OrgParter unitdeptPater = getParterByRule("@is.unitdept@default");
						OrgParterValue udv = new OrgParterValue(unitdeptPater, unituid);
						allAuths.add(udv);
						
						
						//通过单位部门UID找到单位
						dos = DOService.getService("do_org_unit_dept_browse_byuid");
						ls = dos.invokeSelect(unituid);
						if(ls.size()>0){
							bi = (BOInstance)ls.get(0);
							unituid = bi.getValue("unituid");
							if(unituid !=null && !unituid.trim().equals("")){
								//添加单位权限验证
								OrgParter unitPater = getParterByRule("@is.unit@default");
								OrgParterValue uv = new OrgParterValue(unitPater, unituid);
								allAuths.add(uv);
							}
						}
					}
	...
	public static OrgParter getParterByRule(String rule)
    {
        OrgParter ret = null;
        for(Iterator iterator = OrgParter.getAllParters().iterator(); iterator.hasNext();){
        	OrgParter orgparter = (OrgParter)iterator.next();
        	if(orgparter.getRules().startsWith(rule)){
        		ret = orgparter;
        		break;
        	}
        }
