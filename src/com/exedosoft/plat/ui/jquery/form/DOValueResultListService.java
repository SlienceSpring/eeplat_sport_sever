...
public class DOValueResultListService extends DOBaseForm {
		...
		if (theValue != null && !"".equals(theValue.trim())) {
			...
			if (theValue.indexOf(";") == -1 && theValue.indexOf(",") == -1) {
				...
				if (bi != null) {
					...
					if(bi.getName()!=null){
						return DOValueService.stardardSelectPane(property,bi.getName()).toString();
					}else{
						return DOValueService.stardardSelectPane(property,property.getValue()).toString();
					}
					
				}
			} else {
				return mutliBOInstances(property, theValue, corrBO);
	...
	public String mutliBOInstances(DOFormModel property, String theValue,
			DOBO corrBO) {
		String[] strs = null;
