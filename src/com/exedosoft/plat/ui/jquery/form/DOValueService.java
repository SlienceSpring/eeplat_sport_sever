...
public class DOValueService extends DOBaseForm {
	...
	private static StringBuffer stardardOnlyPane(DOFormModel fm, String label,
		...
		appendJSPaneLink(fm, buffer, isSelectPane);
	...
	public static void appendJSPaneLink(DOFormModel fm, StringBuffer buffer, boolean isSelectPane) {

		BOInstance bi = fm.getData();

		buffer.append("javascript:invokeDomId='").append(fm.getFullColID())
				.append("',loadPml({'pml':'");

		if (fm.getLinkPaneModel().getLinkType() != null
				&& fm.getLinkPaneModel().getLinkType().intValue() == DOPaneModel.LINKTYPE_RESOURCE) {
			buffer.append(fm.getLinkPaneModel().getResource().getResourcePath());
		} else {
			buffer.append(fm.getLinkPaneModel().getName());
		}

		if (bi != null) {
			buffer.append("','paras':'dataBus=setContext&contextKey=")
					.append(bi.getBo().getName()).append("&contextValue=")
					.append(bi.getUid());
		...
		if (fm.getLinkPaneModel().getPaneWidth() != null) {
			buffer.append("','pmlWidth':'").append(
					fm.getLinkPaneModel().getPaneWidth());
		}

		if (fm.getLinkPaneModel().getPaneHeight() != null) {
			buffer.append("','pmlHeight':'").append(
					fm.getLinkPaneModel().getPaneHeight());
		}
		...
		if (fm.getTargetPaneModel() != null) {
			...
			// 采用界面元素所在的面板
			if(isSelectPane){///如果选择页面，肯定是弹出形势，其它情况采用界面风格默认，default的风格是弹出,bootstrap是_opener_tab
				targetPaneId = "_opener";// //////valueService 采用弹出的方式
			}
		}

		buffer.append(",'target':'").append(targetPaneId).append("'");
		buffer.append("})");
	}
	...
	public static StringBuffer standardPlugInService(DOFormModel fm,
		...
		if (fm.getLinkPaneModel() != null
			...
			if (fm.getData() != null) {
				...
				href.append("?contextInstanceUid=")
						.append(fm.getData().getUid())
						.append("&contextClassUid=")
	...
	private static String getIcon(String icon, String tooltip) {
		...
		buffer.append("<img src=").append(aPath).append(" border='0'")
				.append(" alt='").append(tooltip).append("' />");
