 ...
 * 
 */
public class LoadTreeSvl20151214 extends javax.servlet.http.HttpServlet implements
	...
	private static Log log = LogFactory.getLog(LoadTreeSvl20151214.class);
	...
	private String dealChildMult(DOTreeModel treeModel, String instanceUid) {
		...
		if(treeModel.getService().retrieveParaServiceLinks().size() == 2){
			list = treeModel.getService().invokeSelect(instanceUid,instanceUid);
		}else{
			list = treeModel.getService().invokeSelect(instanceUid);
		}		
...
//		}
	...
	private void dealChildOneHelper(DOTreeModel treeModel, String instanceUid,
		...
		for (Iterator it = list.iterator(); it.hasNext();) {
			...
			List listChilds = null;
			
			if(childModel.getService().retrieveParaServiceLinks().size() == 2){
				listChilds = childModel.getService().invokeSelect(instanceUid,instanceUid);
			}else{
				listChilds = childModel.getService().invokeSelect(instanceUid);
			}
