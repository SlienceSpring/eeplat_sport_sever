 ...
 * @author aa
 */
public class GridListChild extends DOViewTemplate {

	private static Log log = LogFactory.getLog(GridListChild.class);

	public GridListChild() {

		dealTemplatePath("/grid/GridListChild.ftl");
	}
	...
	public Map<String, Object> putData(DOIModel doimodel) {
		...
		if (gm.getService() != null &&  gm.getService().getBo().getMainPaneModel()!=null) {
			...
			data.put("mainPaneName", "PM_" + gm.getService().getBo().getName() + "_browse");
		}
		return data;
	}
	...
	public List<BOInstance> getListData(DOGridModel gridModel,
		...
		//这个pageno 要和 pagiPml结合，如果有pagiPml的情况，要考虑当前的面板和pagiPml的面板是否一直。 现阶段值考虑一个子列表的分页状态
		...
		if (DOGlobals.getInstance().getSessoinContext().getFormInstance() != null
			...
			try {
				pageNo = Integer.parseInt(DOGlobals.getInstance()
						...
						.getValue("pageNo"));
				///////////如果当前的面板和changPml面板不一致。
				String pagiPml = DOGlobals.getInstance().getSessoinContext()
						.getFormInstance().getValue("pagiPml");
				if(pagiPml!=null){
		...
		int pageSize = 1;
		if(pageNum > 0){
			pageSize = StringUtil.getPageSize(resultSize, pageNum);
		}
