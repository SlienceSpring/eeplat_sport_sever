...
public class DOValueResultList extends DOBaseForm {
			...
			if (theValue.indexOf(";") == -1 && theValue.indexOf(",") == -1) {
				...
				if (bi != null) {
					...
					String dealBus = "dataBus=setContext&contextKey="
							+ bi.getBo().getName() + "&contextValue="
							+ bi.getUid();

					if (theModel == null) {
						theModel = bi.getBo().getMainPaneModel();
					}
					if (theModel != null) {

						StringBuffer buffer = new StringBuffer(
								"<a href=\"javascript:loadPml({'pml':'");
						buffer.append(theModel.getName())
								.append("','target':'_opener_tab','paras':'")
								.append(dealBus).append("'}");
						buffer.append(")\"> ").append(bi.getName())
								.append("</a>");
				...
				return multiInstances(fm, theValue, corrBO);
	...
	public String multiInstances(DOFormModel fm, String theValue, DOBO corrBO) {
		String[] strs = null;
	...
	public static BOInstance getAInstance(DOFormModel property, DOBO corrBO,
		...
		if (property != null) {
			...
			if (secondService != null) {
				System.out.println("The line data:::" + property.getData() );
				List lists = secondService.select(property.getData());
				if(lists!=null){
					bi = (BOInstance)lists.get(0);					
				}
			} else {
