...
/**
 * 微信的列表有两种实现思路，一种是设定表格的类型。
 * 表格元素依然采用传统的组件。
 * 另外一种是通过新的界面组件来定义表格，目前采用的是第二种。
 * @author kevin
 *
 */
public class GridList extends DOViewTemplate {
	...
	public Map<String, Object> putData(DOIModel doimodel) {
		...
		if (gm.getContainerPane() != null && gm.getContainerPane().getParent() != null) {
			...
			// //自动判断条件面板
			List children = gm.getContainerPane().getParent().retrieveChildren();
			...
			if (children != null && children.size() == 2) {
				...
				if (conditionPane != null) {
					if (conditionPane.getDOGridModel() != null) {
						formName = "a" + conditionPane.getDOGridModel().getObjUid();
					}
				}
		...
		if (gm.getService() != null && gm.getService().getBo().getMainPaneModel() != null) {
			String mainPaneName = gm.getService().getBo().getMainPaneModel().getName();
			data.put("mainPaneName", mainPaneName);
		}
		
		data.put("onlyData",false);
		if("true".equals( DOGlobals.getInstance().getSessoinContext().getFormInstance().getValue("onlyData"))){
			data.put("onlyData",true);
		}
		return data;
	}
	...
	public List<BOInstance> getListData(DOGridModel gridModel, Map<String, Object> data) {
		...
		if (DOGlobals.getInstance().getSessoinContext().getFormInstance() != null
				&& DOGlobals.getInstance().getSessoinContext().getFormInstance().getValue("pageNoStatus") != null) {
			try {
				pageNoStatus = Integer
						.parseInt(DOGlobals.getInstance().getSessoinContext().getFormInstance().getValue("pageNoStatus"));
			} catch (Exception e) {
		...
		if (pageNum <= 0) {
			...
			data.put("pageNoStatus", pageNoStatus);

			if ("0".equals(gridModel.getTemplate()) && !isDirectLink) {
				list = new ArrayList();
			} else {
				if(pageNoStatus!=0){
					list = gridModel.getService().invokeSelect(1, pageNum * pageNoStatus);
				}else{
