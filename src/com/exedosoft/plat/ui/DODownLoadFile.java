...
public class DODownLoadFile {
	...
	public static void outStream(String bouid, String thisuuid, String name,
		...
		sql.append(name);
		sql.append(" from ").append(bo.getSqlStr()).append(" where ")
				.append(bo.getKeyCol()).append(" = ?");
	...
	public static String outHtmlCodeByName(String paneModelName) {

		DOPaneModel pm = DOPaneModel.getPaneModelByName(paneModelName);
		if (pm.getLinkType() != null
				&& pm.getLinkType().intValue() == DOPaneModel.LINKTYPE_GRIDMODEL) {
			if (pm.getLinkUID() != null && !"".equals(pm.getLinkUID().trim())) {
				DOGridModel gm = DOGridModel.getGridModelByID(pm.getLinkUID());
	...
	public static String outDataTemplate(String tableName) {

		StringWriter strSW = new StringWriter();
		CSVWriter cw = new CSVWriter(strSW);
		...
		for (DOBOProperty aProp : props) {

			if (!aProp.isKeyCol()
					&& (aProp.getPersType() == null || aProp.getPersType()
							.intValue() == DOBOProperty.PERSISTENCE_YES)) {

				listHeader.add(aProp.getColName());
			}
		}
		all.add(listHeader.toArray(new String[listHeader.size()]));

		DOService service = bo.getDSeleAllService();

		if (service != null) {
			List<BOInstance> data = service.invokeSelect(1, 10);
			for (BOInstance bi : data) {
				String[] aLine = new String[listHeader.size()];
				for (int i = 0; i < listHeader.size(); i++) {
					aLine[i] = bi.getValue4Null(listHeader.get(i));
				}
				all.add(aLine);
			}
		}
	...
	private static void downLoadSizeCol(DOBO bo, String downLoadSizeCol,
		...
		sql.append(" set ").append(downLoadSizeCol).append("=")
				.append(downLoadSizeCol).append("+1 where ")
				.append(bo.getKeyCol()).append("=?");
	...
	public static void main(String[] args) {
		...
		System.out.println("Customer:: " + DODownLoadFile.outDataTemplate("crm_customer") );
