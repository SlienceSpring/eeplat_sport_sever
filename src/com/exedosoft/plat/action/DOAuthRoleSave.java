...
public class DOAuthRoleSave extends DOAbstractAction {
	...
	private void baseAuth(String parterUid, List allAuthObjs, String strKey,
		...
		try {
			for (int i = 0; i < configs.length; i++) {
				...
				if (aConfig[0] != null && !"".equals(aConfig[0])) {
					...
					if(DOAuthorization.WHAT_APPLICATION==type){
						DOApplication dop = DOApplication.getApplicationByID(aConfig[0]);
						if(dop!=null && !dop.isFilter()){
							continue;
						}
					}
					...
					if (allAuthObjs != null
							&& allAuthObjs.contains(containKey)) {
