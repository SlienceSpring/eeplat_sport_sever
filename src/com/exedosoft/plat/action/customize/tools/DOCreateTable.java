...
public class DOCreateTable extends DOAbstractAction {
	...
	public String excute() {
		...
		String[] remarks = form.getValueArray("col_remark");
		...
		sb.append(tableName.replaceAll("\\s", "")).append(" (");
		for (int i = 0; i < colNames.length; i++) {

			String colName = colNames[i];
			if (!colName.equals("")) {
				sb.append(colName.replaceAll("\\s", "")).append("  ").append(dbtypes[i]);

				if (dbsizes[i] != null && !dbsizes[i].equals("")) {
					sb.append(" (").append(dbsizes[i]).append(")");
				}

				if(dss.isMySql() || ("h2".equals(dss.getDialect()))){
					sb.append("	COMMENT '")
					.append(remarks[i])
					.append("' ");
				}
