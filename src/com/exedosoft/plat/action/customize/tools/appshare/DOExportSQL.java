...
public class DOExportSQL extends DOAbstractAction {
		...
		try {
			...
			if (shareUser != null) {
				...
				for (String tableName : allBO) {
					...
					String patterm = tableName;//
					Pattern p = Pattern.compile(patterm,
							Pattern.CASE_INSENSITIVE);
					Matcher m = p.matcher(oldSrc);
					while (m.find()) {
						m.appendReplacement(nb, userID+ "_" + m.group(0));
					}
					m.appendTail(nb);
					oldSrc = nb.toString();
				}
	...
	private void appendBOInstanceHelper(StringBuilder sb, BOInstance bi,
		...
		for (Iterator<Entry<String, Object>> itEntry = bi.getMap().entrySet()
			...
			if (isReplace) {
				sbValues.append("'").append(replaceShareUserId(value))
						.append("'");
			} else {
...
//				.invokeSelect("ff8080813af2be9e013af32668830079");
//		a.appendLi(sb, ruleServices);
//
//		System.out.println("SB:::" + sb);
		String oldSrc = "abcd  do_bo_a  ccc  eee    do_bo_property do_BO_a_ccc"; 
		
		List<String> allBO = new ArrayList();
		allBO.add("do_bo_a");
		allBO.add("do_bo_property");
		...
		for (String tableName : allBO) {
			...
			while (m.find()) {
				...
				m.appendReplacement(nb, "eeplat" + "_" + m.group(0));
		...
		System.out.println("OldStr:::" + oldSrc);
