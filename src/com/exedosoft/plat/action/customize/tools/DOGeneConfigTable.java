...
public class DOGeneConfigTable extends DOAbstractAction {
	...
	public String excute() {
		...
		String geneType = this.actionForm.getValue("geneType");
		String[] keyCols = this.actionForm.getValueArray("keyCol");
		String[] valueCols = this.actionForm.getValueArray("valueCol");
		String[] labels = this.actionForm.getValueArray("tablelabel");
		String[] hiddenTables = this.actionForm
				.getValueArray("checkinstance_hidden");

		if (tables == null || tables.length == 0) {
			this.setEchoValue(I18n.instance().get("没有选择表!"));
			return NO_FORWARD;
		}

		// //查看业务包是否存在

		DOBO bpBO = DOBO.getDOBOByName("do_busipackage");
		BOInstance bpBI = bpBO.getInstance(bpUid);

		if (bpBI == null) {
			this.setEchoValue(I18n.instance().get("没有选择包，或选择不正确!"));
			return NO_FORWARD;
		}
				...
				if (aHiddenTable.equals(tables[check])) {
					...
					if (i < keyCols.length) {
						if (keyCols[i] != null && !keyCols[i].trim().equals("")) {
							...
							aIns.putValue("tablelabel", labels[i].trim());
		...
		try {
			...
			for (java.util.Iterator<BOInstance> it = allCheckInstances
				...
				String tabelLabel = bi.getValue("tablelabel");
				String keycol = bi.getValue("keycol");
				String valuecol = bi.getValue("valuecol");
				
				
				if (geneType.equals("1")) {
					if (valuecol == null || valuecol.trim().equals("")) {
						valuecol = keycol;
					}
				}

				ATableForwarderJquery af = new ATableForwarderJquery(aTable,tabelLabel,
						keycol, valuecol, dataSourceUid, bpUid, "");
				af.setGeneType(geneType);
				af.forwardAll();
			}
