...
public class TransFormModelL10n extends DOAbstractAction {
	...
	public String excute() throws ExedoException {
		...
		DOService propertyUpdateL10n = DOService.getService("DO_BO_Property_transl10n");
...
//			}
			...
			if(propertyUpdateL10n!=null){
				propertyUpdateL10n.beginBatch();
				for(int i = 0; i < tranArray.length; i++){
					String aLine = tranArray[i];
					if(aLine==null){
						break;
					...
					if (paras != null && paras.length > 1) {
						...
						map.put("l10n_en", paras[0]);
						map.put("l10n_china", paras[1].replaceAll("\\r", ""));
						propertyUpdateL10n.addBatch(map);
					}
				}
				propertyUpdateL10n.endBatch();				
			}
		} catch (Exception e) {
			e.printStackTrace();
		}
...
//		}
		...
		DOService service = DOService.getService("do_org_role_list");
		
		List list = (List)service.invokeSelect();
