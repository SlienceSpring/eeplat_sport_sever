...
public class DOTableList extends DOAbstractAction {
	...
	public String excute() {
		...
		if ("true".equals(DOGlobals.getValue("multi.tenancy"))) {
			...
			dss = DODataSource.getDataSourceByID(bo.getCorrInstance().getUid());
		}
		...
		try {
			...
			if (DODataSource.DIALECT_SQLSERVER.equals(dss.getDialect())) {
				
				PreparedStatement pstmt = con.prepareStatement("Select NAME FROM  sysobjects  Where (XType='U' or XTYPE='V') order BY Name");
