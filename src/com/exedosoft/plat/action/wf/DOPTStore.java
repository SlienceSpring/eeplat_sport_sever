...
public class DOPTStore extends DOAbstractAction {
	...
	public String excute() throws ExedoException {
		...
		try {
			...
			DOService insertNodeService = DOService
					.getService("do_pt_node_insert");
			...
			DOService getNodeDByUid = DOService
					.getService("do_pt_node_denpendency_browse");

			StringReader read = new StringReader(ptXml);
			// 创建新的输入源SAX 解析器将使用 InputSource 对象来确定如何读取 XML 输入
			InputSource source = new InputSource(read);

			Document doc = DOMXmlUtil.getInstance().getDocumentBuilder()
					.parse(source);
			...
			for (int i = 0; i < nList.getLength(); i++) {
				...
				if (node.getNodeType() == Document.ELEMENT_NODE) {
					...
					List existNodes = getNodeByUid.invokeSelect(e
							.getAttribute("id"));
					...
					try {
						if (e.getAttribute("authType") != null
								&& !e.getAttribute("authType").trim()
										.equals("")) {
							ptNode.setAuthType(Integer.parseInt(e
									.getAttribute("authType")));
						}

						if (e.getAttribute("decisionType") != null
								&& !e.getAttribute("decisionType").trim()
										.equals("")) {
							ptNode.setDecisionType(Integer.parseInt(e
									.getAttribute("decisionType")));
						}
					} catch (Exception e1) {
					...
					if (paneName != null && !paneName.trim().equals("")) {
						DOPaneModel aPM = DOPaneModel
								.getPaneModelByName(paneName);
						if (aPM != null) {
							ptNode.setPane(aPM);
						}
					}
					...
					ptNode.setXmlSchema(e.getAttribute("subflow"));
			...
			for (int i = 0; i < nList.getLength(); i++) {
				...
				if (node.getNodeType() == Document.ELEMENT_NODE) {
					Element e = (Element) node;
					List exists = getNodeDByUid.invokeSelect(e
							.getAttribute("id"));
