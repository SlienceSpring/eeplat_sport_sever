...
public class GeneUIParentChild {
	...
	public void geneConfigBrowse() {
		...
		try {
			 ...
			 */
			DOService findByParentService = geneFindByParentService();
			DOGridModel childGrid = genePaneAndGrid(childCategory, gridList,
					childATable + "_BrowseChild", childTitle, parentGrid,
					findByParentService, true);
	...
	public DOService geneFindByParentService() throws ExedoException {
		...
		String serviceName = childATable + "_findbyparent";
		// ///////////////////////////TEAdd
		...
		if (parentGrid != null) {
			...
			if (childInsertService == null) {
				childInsertService = copyService(childInsertServiceDefault,
						newInsertServiceName, this.childCategory.getObjUid(),
						this.geneATable, this.linkCol);
			}
			/**
			 * 第一服务是新增
			 */
			...
			if (childModifyService == null) {
				childModifyService = copyService(childModfiyServiceDefault,
						newModfiyServiceName, this.childCategory.getObjUid(),
						this.geneATable, this.linkCol);
			}
	...
	public static DOService copyService(DOService oldService,
			String newServiceName, String childBOUid,
			String parentTableName, String linkCol) {

		linkCol = linkCol.toLowerCase();
		BOInstance biService = new BOInstance();
		biService.fromObject(oldService);

		DOBO parentBO = DOBO.getDOBOByName(parentTableName);
		...
		try {
			...
			biService.putValue("name", newServiceName);
			biService.putValue("l10n", newServiceName);
			biService.putValue("bouid", childBOUid);
			if(newServiceName.indexOf("_update") > 0){
				String mainSql = oldService.getMainSql();
				mainSql  = mainSql.replace(" "+linkCol+"=?,", " ");
				mainSql  = mainSql.replace(","+linkCol+"=?", "");
				biService.putValue("mainSql", mainSql);
			}
			
			
			BOInstance biNewService = thisService.invokeUpdate(biService);

			for (Iterator<DOParameterService> it = oldService
				...
				if (dps.getDop().getName().equalsIgnoreCase(linkCol)) {
					...
					DOBOProperty property = DOBOProperty.getDOBOPropertyByName(
							parentTableName,  parentBO.getKeyCol());
			...
			for (Iterator<DOServiceRule> it = oldService.retrieveServiceRules()
					.iterator(); it.hasNext();) {
	...
	public static void main(String[] args) throws ExedoException {
		...
		String mainSql = "update crm_activity_contact set activitity_uid=?,type=?,contact_uid=? where objuid = ?";
		
		mainSql  = mainSql.replace("\\s"+"activitity_uid"+"=?,", " ");
		mainSql  = mainSql.replace(","+"activitity_uid"+"=?", "");
