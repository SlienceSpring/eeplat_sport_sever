...
public class PropertyManager {
	...
	public void buildRelation(String thisboUid, String propertyName, DOBO aBO, int relationType) {
		...
		t.begin();
		List<DOBOProperty> lists = DAOUtil
				.INSTANCE()
				.select(DOBOProperty.class,
						"select * from do_bo_property  where col_name = ? and boUid = ?",
						propertyName, thisboUid);
		...
		try {
			for (Iterator<DOBOProperty> it = lists.iterator(); it.hasNext();) {
				...
				// / 干脆把连接去掉试试。 又放开 @20150908
				if (dop.getLinkBO() == null) {
					dop.setLinkBO(aBO);
					
					///关联1  组合2
					dop.setRelationType(relationType);
					DAOUtil.INSTANCE().store(dop);
				}
				...
				for (Iterator<DOFormModel> itFm = forms.iterator(); itFm
					...
					if (fm.getGridModel().getName().toLowerCase()
							...
							|| fm.getGridModel().getName().toLowerCase()
									.contains("_result")
							|| fm.getGridModel().getController().getName()
									.toLowerCase().contains("list")
							|| fm.getGridModel().getController().getName()
									.toLowerCase().contains("details")) {
						...
						DOService theConditionService =  DOService.getService(aBO.getName()
								+ "_auto_criteria", aBO);
						fm.setLinkService(theConditionService);
						fm.setController(DOController
	...
	public void removeRelation(String thisboUid, String propertyName, DOBO aBO, int relationType) {
			...
			for (Iterator<DOBOProperty> it = lists.iterator(); it.hasNext();) {
				...
				for (Iterator<DOFormModel> itFm = forms.iterator(); itFm
					...
					if (fm.getGridModel().getName().toLowerCase()
							...
							|| fm.getGridModel().getController().getName()
									.toLowerCase().contains("details")) {
						fm.setLinkService(null);
						fm.setController(DOController
								.getControllerByName(DOValueSimple.class
										.getCanonicalName()));
					} else {
						
						fm.setController(DOController
								.getControllerByName(DOInputText.class
										.getCanonicalName()));
					}
					DAOUtil.INSTANCE().store(fm);
					// }
	...
	public static void main(String[] args) throws ExedoException {
		...
		// relationPropertyUid
		pm.buildRelation("DO_BusiPackage", "applicationUID", boApplication, 1);
