...
public class GeneUIMain {
	...
	public static final String GENETYPE_NORMAL = "0";

	// ///////只设这两种，不设第三种了，扩展视图就是这个样子。保持简单，以后以 bootstrap风格为主了。
	public static final String GENETYPE_EXTENDVIEW = "2";
	...
	private DOController formItemPane = DOController
			.getControllerByClassName(TPaneSelected.class.getName());

	private DOController formItemServiceUf = DOController
			.getControllerByClassName(TServiceSelectedUf.class.getName());
	...
	public GeneUIMain(String aTable, String aLabel) {
	...
	private DOPaneModel geneCondition(List properties, DOPaneModel pmResult)
		...
		fm.setController(formPaneSearch);
		fm.setGridModel(conditionGrid);
		fm.setOrderNum(Integer.valueOf(i));
	...
	private DOPaneModel geneResult(List properties) throws ExedoException {
		...
		if ("en".equals(DOGlobals.getValue("lang.local"))) {
			gmResult.setCaption(this.label);
