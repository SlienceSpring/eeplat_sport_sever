...
public class NodeInstance extends BaseObject implements Serializable {
	...
	private void dealPropetyValues(BOInstance formI) {
		...
		if (node != null) {
			...
			if (node.getXmlSchema() != null
					&& !"".equals(node.getXmlSchema().trim())) {
				this.setRetNodeUID(formI.getValue(node.getXmlSchema()));
			}
	...
	public void execute() throws WFException {
		...
		if (getNodeType() != null
			...
			String subFlowName = this.getXmlSchema();
	...
	public static NodeInstance initNodeInstance(ProcessInstance pi,
		...
		ni.setXmlSchema(node.getXmlSchema());
		ni.setX(node.getX());
		ni.setY(node.getY());
	...
	private void storeNextNodeAuthorization(NodeInstance nextNodeInstance) {
		...
		if (nextNodeInstance.getAuthType() != null
				&& (nextNodeInstance.getAuthType().intValue() == PTNode.AUTH_TYPE_SCHEDULE_USER || nextNodeInstance
						...
						|| 
					 nextNodeInstance
						.getAuthType().intValue() == PTNode.AUTH_TYPE_DATA_OWNER  ///add by wkx @20160107
						)) {
...
//			}
			...
			try {///add by wkx @20160107
				if(nextNodeInstance
						.getAuthType().intValue() == PTNode.AUTH_TYPE_DATA_OWNER){
					nextPerformerUids = this.getProcessInstance().getBOInstance().getValue(this.getSpecName());
				}
			} catch (Exception e) {
				// TODO Auto-generated catch block
				e.printStackTrace();
